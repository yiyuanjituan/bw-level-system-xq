          ...
          <div class="content-box">
            <x-form :rule="formRules" :model="modelValue" ref="form">
              <x-form-item prop="bank_id" v-if="thirdList.filter(v => v.type == 0).length > 0">
                <x-select prefix="input_icon_bank" :options="thirdList.filter(v => v.type == 0)" value-key="id" label-key="wallet_name" icon-key="wallet_icon" v-model="modelValue.bank_id" :placeholder="$t('请选择钱包')">
                  <template #prefix>
              ...
              <x-form-item prop="bank_number">
                <x-input :placeholder="$t('请输入钱包地址')" v-model="modelValue.bank_number" type="text">
                  <template #suffix>
