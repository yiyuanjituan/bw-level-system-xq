...
import { getPromoteSubordinateFinance } from "@/api/common";
...
const selectedSort = ref("deposit");
const memberId = ref("");
...
const cursorMemberId = ref<number | null>(null);
const members = ref<PromoteSubordinateFinance[]>([]);
const summary = ref<PromoteSubordinateFinanceResponse | null>(null);
...
const footerItems = computed<FooterItem[]>(() => {
  ...
  return [
    {
      label: "总充值",
      value: formatMoney(current?.totalDeposit),
      count: formatCount(current?.totalDepositCount),
      unit: "次"
    },
    {
      label: "总首充",
      value: formatMoney(current?.totalFirstDeposit),
      count: formatCount(current?.totalFirstDepositPerson),
      unit: "人"
    },
    {
      label: "总提现",
      value: formatMoney(current?.totalWithdraw),
      count: formatCount(current?.totalWithdrawCount),
      unit: "次"
    }
  ];
});
...
function normalizeMember(source: Record<string, unknown>): PromoteSubordinateFinance {
  const deposit = Number(source.deposit) || 0;
  const withdraw = Number(source.withdraw) || 0;

  return {
    userIdx: formatCount(source.userIdx),
    vipLevel: formatCount(source.vipLevel),
    directChildCount: formatCount(source.directChildCount),
    deposit,
    depositCount: formatCount(source.depositCount),
    withdraw,
    withdrawCount: formatCount(source.withdrawCount),
    difference: Number(source.difference) || deposit - withdraw,
    balance: Number(source.balance) || 0
  };
}

function normalizeResponse(response: unknown): PromoteSubordinateFinanceResponse {
  const source = ((response as { data?: unknown })?.data ?? response ?? {}) as Record<string, unknown>;
  const rawList = Array.isArray(source.list) ? source.list : [];
  const more = Boolean(source.more);
  const nextCursorValue = Number(source.nextCursorValue);
  const nextCursorMemberId = formatCount(source.nextCursorMemberId);
  const hasNextCursor = more && Number.isFinite(nextCursorValue) && nextCursorMemberId > 0;

  return {
    list: rawList.map(member => normalizeMember(member as Record<string, unknown>)),
    pageSize: formatCount(source.pageSize) || DEFAULT_PAGE_SIZE,
    more: hasNextCursor,
    nextCursorValue: hasNextCursor ? nextCursorValue : null,
    nextCursorMemberId: hasNextCursor ? nextCursorMemberId : null,
    totalDeposit: Number(source.totalDeposit) || 0,
    totalDepositCount: formatCount(source.totalDepositCount),
    totalFirstDeposit: Number(source.totalFirstDeposit) || 0,
    totalFirstDepositPerson: formatCount(source.totalFirstDepositPerson),
    totalWithdraw: Number(source.totalWithdraw) || 0,
    totalWithdrawCount: formatCount(source.totalWithdrawCount),
    startTime: Number(source.startTime) || 0,
    endTime: Number(source.endTime) || 0
  };
}

function createQueryParams(): PromoteSubordinateFinanceParams {
  ...
  return {
    ...
    ...(cursorValue.value !== null && cursorMemberId.value !== null
      ? {
          cursorValue: cursorValue.value,
          cursorMemberId: cursorMemberId.value
        }
      : {}),
...
async function loadMembers(reset = false) {
  ...
  loading.value = true;
  try {
    const response = normalizeResponse(await getPromoteSubordinateFinance(createQueryParams()));
    if (requestId !== latestRequestId) return;

    members.value = reset ? response.list : [...members.value, ...response.list];
    summary.value = response;
    requestFailed.value = false;
    finished.value = !response.more;
    cursorValue.value = response.nextCursorValue;
    cursorMemberId.value = response.nextCursorMemberId;
  } catch {
    if (requestId !== latestRequestId) return;
      ...
      >
        ...
        <div v-for="member in members" :key="member.userIdx" class="member-item">
          ...
          <div class="member-row">
            <span class="member-column">
              <label>充值</label>
              <span class="number value">{{ formatMoney(member.deposit) }}</span>
              <span>({{ formatCount(member.depositCount) }}次)</span>
            </span>
            <span class="member-column">
              <label>提现</label>
              <span class="number value">{{ formatMoney(member.withdraw) }}</span>
              <span>({{ formatCount(member.withdrawCount) }}次)</span>
            </span>
          </div>
          <div class="member-row">
            <span class="member-column">
              <label>差额</label>
              <span class="number value" :class="getDifferenceClass(member.difference)">
                {{ formatMoney(member.difference) }}
              </span>
            </span>
            <span class="member-column">
              <label>余额</label>
              <span class="number value">{{ formatMoney(member.balance) }}</span>
            </span>
          </div>
        </div>
      ...
      <div class="footer-content">
        <span v-for="footerItem in footerItems" :key="footerItem.label" class="footer-item">
          ...
          <span class="footer-value">
            <span class="number">{{ footerItem.value }}</span>
            <span class="footer-count">
              (<span class="number">{{ footerItem.count }}</span>{{ footerItem.unit }})
            </span>
          </span>
        </span>
      </div>
